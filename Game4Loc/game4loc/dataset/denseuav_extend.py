...
class ConditionalResize(A.ImageOnlyTransform):
    def __init__(self, min_size=768, always_apply=False, p=1.0):
    ...
    def apply(self, img, **params):
        ...
        if h < self.min_size or w < self.min_size:
            img = cv2.resize(img, (self.min_size, self.min_size), interpolation=cv2.INTER_LINEAR)
        return img
...
class DenseUAVEDatasetTrain(Dataset):
    
    def __init__(self,
                 query_folder,
                 query_extend_folder,
                 gallery_folder,
                 gallery_extend_folder,
                 pair_de_s=None,
                 pair_d_se=None,
                 pair_de_se=None,
        ...
        self.query_extend_dict = None
        self.gallery_extend_dict = None
        if query_extend_folder:
            self.query_extend_dict = get_data(query_extend_folder)
            self.ids = self.ids.intersection(self.query_extend_dict.keys())
        if gallery_extend_folder:
            self.gallery_extend_dict = get_data(gallery_extend_folder)
            self.ids = self.ids.intersection(self.gallery_extend_dict.keys())
        ...
        for idx in self.ids:
            ...
            if self.gallery_extend_dict != None:
                gallery_extend_imgs = self.gallery_extend_dict[idx]["files"]
                if pair_de_s:
                    for q in self.query_dict[idx]["files"]:
                        for g in gallery_extend_imgs:
                            self.pairs.append((idx, q, g))

            if self.query_extend_dict != None:
    ...
    def shuffle(self, ):
            ...
            while True:
                ...
                if len(current_batch) >= self.shuffle_batch_size:
                    ...
                    batches_jyx.append(current_batch)
            ...
            for batch in batches_jyx:
                idx_list = []
                for (idx, _, _) in batch:
                    idx_list.append(idx)
                if len(batch) != len(set(idx_list)):
                    print('jyxjyx batch', len(batch), len(set(idx_list)))
...
def get_transforms(img_size,
    ...
    train_sat_transforms = A.Compose([# ConditionalResize(min_size=768),
                                      # A.CenterCrop(height=768, width=768, p=1.0),
    ...
    train_drone_transforms = A.Compose([# ConditionalResize(min_size=768),
                                        # A.CenterCrop(height=768, width=768, p=1.0),
                                        A.ImageCompression(quality_lower=90, quality_upper=100, p=0.5),
                                        A.Resize(img_size[0], img_size[1], interpolation=cv2.INTER_LINEAR_EXACT, p=1.0),
                                        A.ColorJitter(brightness=0.15, contrast=0.15, saturation=0.15, hue=0.15, always_apply=False, p=0.5),
...
if __name__ == "__main__":
    import cv2
    a = cv2.imread('/home/xmuairmud/data/University-Release/train/satellite_outpainting/1615/1615-50-212.jpg')
    conResize = ConditionalResize(min_size=768)
    a = conResize(image=a)['image']
    print(a.shape)

    a = cv2.imread('/home/xmuairmud/data/University-Release/train/satellite/1615/1615.jpg')
    a = conResize(image=a)['image']
    print(a.shape)
