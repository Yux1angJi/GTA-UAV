...
class U1652EDatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        self.ids = list(set(self.query_dict.keys()).intersection(self.gallery_dict.keys()).intersection(self.gallery_extend_dict.keys()).intersection(self.query_extend_dict.keys()))
        self.ids.sort()
        ...
        for idx in self.ids:
            ...
            if pair_de_s:
                for g in gallery_extend_imgs:
                    self.pairs.append((idx, query_img, "{}/{}".format(gallery_extend_path, g)))

            query_extend_path = self.query_extend_dict[idx]["path"]
            query_extend_imgs = self.query_extend_dict[idx]["files"]

            for q in query_extend_imgs:
                query_img = "{}/{}".format(query_extend_path, q)
